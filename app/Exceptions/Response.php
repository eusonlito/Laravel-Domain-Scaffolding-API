...
class Response
{
    ...
    protected function message(): string
    {
        ...
        if ($this->e instanceof AuthenticationExceptionVendor) {
            return __('user-auth.validator.empty');
        }

        if ($this->e instanceof AuthenticationException) {
            return $message ?: __('user-auth.validator.auth');
        }

        if ($this->e instanceof NotFoundHttpException) {
            return $message ?: __('common.validator.not-found');
        }

        if ($this->e instanceof ModelNotFoundException) {
            return $message ?: __('common.validator.not-found-model');
        }

        if ($this->e instanceof MethodNotAllowedHttpException) {
            return __('common.validator.method-not-allowed');
        }

        if ($this->e instanceof QueryException) {
            return __('common.validator.query');
        }

        if ($this->isExceptionSystem()) {
            return __('common.validator.system');
        }
